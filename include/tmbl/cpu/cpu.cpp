...
void cpu::LD(const u8 n, Orientation o) noexcept {
  switch (o) {
    case Orientation::READ_FROM_IO_PORT:
      A = busInstance.read(u16(0xFF00 + n));
      break;
    case Orientation::WRITE_TO_IO_PORT:
      busInstance.write(u16(0xFF00 + n), A);
      break;
  }
...
void cpu::LD(Orientation o) noexcept {
  ...
  switch (o) {
    case Orientation::READ_FROM_IO_PORT:
      A = busInstance.read(u16(0xFF00 + C_reg_val));
      break;
    case Orientation::WRITE_TO_IO_PORT:
      busInstance.write(u16(0xFF00 + C_reg_val), A);
      break;
  }
...
void cpu::LD(const u16 nn, const reg16 rr) noexcept {
  //  m[nn] = rr.lo();
  busInstance.write(nn, rr.lo());
  busInstance.write(nn + 1, rr.hi());

  c.tick(5);
}
void cpu::PUSH(const reg16 rr) noexcept {
  busInstance.write(SP - 1, rr.lo());
  busInstance.write(SP - 2, rr.hi());
...
void cpu::POP(reg16 &rr) noexcept {
  rr.lo(busInstance.read(SP));
  rr.hi(busInstance.read(SP + 1));
  SP += 2;
...
void cpu::ADD(const reg16 rr) noexcept {
  F.N(reset);
  u8 n = busInstance.read(rr);
  ((n + A.value()) == (reg8::max() + 1)) ? F.Z(set) : F.Z(reset);
  ((n + A.loNibble()) > 0b0000'1111) ? F.H(set) : F.H(reset);
  (n + A.value() > reg8::max()) ? F.C(set) : F.C(reset);

  reg8 tmp;
  tmp = busInstance.read(rr);
  A = A + tmp;
...
void cpu::AND(const reg16 rr) noexcept {
  ...
  (A.value() == 0 || busInstance.read(rr) == 0) ? F.Z(set) : F.Z(reset);
...
void cpu::XOR(const reg16 rr) noexcept {
  ...
  (A.value() == busInstance.read(rr)) ? F.Z(set) : F.Z(reset);
...
void cpu::RLC(const reg16 rr) noexcept {
  ...
  busInstance.write(rr, rotl(busInstance.read(rr), 1));
...
void cpu::RL(const reg16 rr) noexcept {
  ...
  busInstance.write(rr, byte(busInstance.read(rr) | mask_for_0th_bit));
...
void cpu::RRC(const reg16 rr) noexcept {
  ...
  busInstance.write(rr, rotr(busInstance.read(rr), 1));
...
void cpu::RR(const reg16 rr) noexcept {
  ...
  busInstance.write(rr, byte((busInstance.read(rr) | mask_for_7th_bit)));
...
void cpu::SLA(const reg16 rr) noexcept {
  ...
  auto val = busInstance.read(rr);
  reg8 tmp;
  tmp = val;

  (tmp.msb() == 1) ? F.C(set) : F.C(reset);

  busInstance.write(rr, val <<= 1);

  (busInstance.read(rr) == 0U) ? F.Z(set) : F.Z(reset);
...
void cpu::SRA(const reg16 rr) noexcept {
  ...
  busInstance.write(rr, val >>= 1);
  busInstance.write(rr, byte(busInstance.read(rr) | old_msb_mask));
  (busInstance.read(rr) == 0U) ? F.Z(set) : F.Z(reset);
...
void cpu::SWAP(const reg16 rr) noexcept {
  ...
  busInstance.write(rr, byte(rotl(busInstance.read(rr), 4)));
...
void cpu::SET(const reg16 rr, const u8 pos) {
  ...
  busInstance.write(rr, byte(busInstance.read(rr) | (1U << pos)));
...
void cpu::CALL(const u16 nn) noexcept {
  busInstance.write(SP - 1, PC.hi());
  busInstance.write(SP - 2, PC.lo());
...
void cpu::RET() noexcept {
  PC.lo(busInstance.read(SP));
  PC.hi(busInstance.read(SP + 1));
  SP = SP + 2;
...
byte cpu::fetch(reg16 rr) { return busInstance.read(rr); }
