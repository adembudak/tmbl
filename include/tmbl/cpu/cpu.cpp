...
void cpu::LD(const u8 n, Orientation o) noexcept {
  switch (o) {
    case Orientation::READ_FROM_IO_PORT:
      A = bus::get().read(u16(0xFF00 + n));
      break;
    case Orientation::WRITE_TO_IO_PORT:
      bus::get().write(u16(0xFF00 + n), A);
      break;
  }
...
void cpu::LD(Orientation o) noexcept {
  ...
  switch (o) {
    case Orientation::READ_FROM_IO_PORT:
      A = bus::get().read(u16(0xFF00 + C_reg_val));
      break;
    case Orientation::WRITE_TO_IO_PORT:
      bus::get().write(u16(0xFF00 + C_reg_val), A);
      break;
  }
...
void cpu::LD(const u16 nn, const reg16 rr) noexcept {
  //  m[nn] = rr.lo();
  bus::get().write(nn, rr.lo());
  bus::get().write(nn + 1, rr.hi());

  c.tick(5);
}
void cpu::PUSH(const reg16 rr) noexcept {
  bus::get().write(SP - 1, rr.lo());
  bus::get().write(SP - 2, rr.hi());
...
void cpu::POP(reg16 &rr) noexcept {
  rr.lo(bus::get().read(SP));
  rr.hi(bus::get().read(SP + 1));
  SP += 2;
...
void cpu::AND(const reg16 rr) noexcept {
  ...
  (A.value() == 0 || bus::get().read(rr) == 0) ? F.Z(set) : F.Z(reset);
...
void cpu::XOR(const reg16 rr) noexcept {
  ...
  (A.value() == bus::get().read(rr)) ? F.Z(set) : F.Z(reset);
...
void cpu::RLC(const reg16 rr) noexcept {
  ...
  bus::get().write(rr, rotl(bus::bus::get().read(rr), 1));
...
void cpu::RL(const reg16 rr) noexcept {
  ...
  bus::get().write(rr, byte(bus::bus::get().read(rr) | mask_for_0th_bit));
...
void cpu::RRC(const reg16 rr) noexcept {
  ...
  bus::get().write(rr, rotr(bus::bus::get().read(rr), 1));
...
void cpu::RR(const reg16 rr) noexcept {
  ...
  bus::get().write(rr, byte((bus::bus::get().read(rr) | mask_for_7th_bit)));
...
void cpu::SLA(const reg16 rr) noexcept {
  ...
  auto val = bus::get().read(rr);
  reg8 tmp;
  tmp = val;

  (tmp.msb() == 1) ? F.C(set) : F.C(reset);

  bus::get().write(rr, val <<= 1);

  (bus::get().read(rr) == 0U) ? F.Z(set) : F.Z(reset);
...
void cpu::SRA(const reg16 rr) noexcept {
  ...
  bus::get().write(rr, val >>= 1);
  bus::get().write(rr, byte(bus::bus::get().read(rr) | old_msb_mask));
  (bus::get().read(rr) == 0U) ? F.Z(set) : F.Z(reset);
...
void cpu::SWAP(const reg16 rr) noexcept {
  ...
  bus::get().write(rr, byte(rotl(bus::bus::get().read(rr), 4)));
...
void cpu::SET(const reg16 rr, const u8 pos) {
  ...
  bus::get().write(rr, byte(bus::bus::get().read(rr) | (1U << pos)));
...
void cpu::CALL(const u16 nn) noexcept {
  bus::get().write(SP - 1, PC.hi());
  bus::get().write(SP - 2, PC.lo());
...
void cpu::RET() noexcept {
  PC.lo(bus::get().read(SP));
  PC.hi(bus::get().read(SP + 1));
  SP = SP + 2;
...
byte cpu::fetch(reg16 rr) { return bus::get().read(rr); }
